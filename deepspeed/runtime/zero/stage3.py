...
class FP16_DeepSpeedZeroOptimizer_Stage3(object):
    ...
    def __init__(self,
        ...
        for i, param_group in enumerate(self.optimizer.param_groups):
            ...
            see_memory_usage("Before Flattening", force=True)
        

            if not self.cpu_offload_params:
                see_memory_usage(f"Before moving param group {i} to CPU", force=False)
                ...
                self.fp16_partitioned_groups_flat.append(
                    flatten_dense_tensors_aligned(
                        self.fp16_partitioned_groups[i],
                        dist.get_world_size(group=self.dp_process_group)).detach())

            # set model fp16 weight to slices of flattened buffer
            updated_params = _unflatten_dense_tensors(self.fp16_partitioned_groups_flat[i],
                                                      self.fp16_partitioned_groups[i])
            ...
            see_memory_usage(f"After creating fp32 copy {i}", force=True)

            # modify optimizer of have flat master weight
            self.fp32_partitioned_groups_flat[
                i].requires_grad = True  # keep this in case internal optimizer uses it
            param_group['params'] = [self.fp32_partitioned_groups_flat[i]]
    ...
    def _register_hooks_recursively(self, module, count=[0]):
        ...
        def _pre_backward_module_hook(module, inputs, output):
            ...
            return PreBackwardFunction.apply(module,
        ...
        def _post_backward_module_hook(module, inputs):
            ...
            requires_grad = any(
                [isinstance(v,
                            torch.Tensor) and v.requires_grad for v in inputs])
            ...
            return PostBackwardFunction.apply(module,
    ...
    def initialize_optimizer_states(self):
        ...
        for i, group in enumerate(self.fp16_groups):
            single_grad_partition = torch.zeros(int(self.fp32_partitioned_groups_flat[i].numel()),
                                                dtype=self.fp32_partitioned_groups_flat[i].dtype,
                                                device=self.device)
            self.fp32_partitioned_groups_flat[i].grad = single_grad_partition.pin_memory() if self.cpu_offload else single_grad_partition
        ...
        for i, param_group in enumerate(self.fp16_groups):
            for param in param_group:
                if param.requires_grad:
                    print(f" Before all gather {param.device}, {param.shape}")
    ...
    def set_grad_positions(self):
        for i, group in enumerate(self.fp16_groups):
            ...
            for param in group:
                ...
                self.grad_position[param_id] = [int(i),
                                                int(current_offset),
                                                int(num_elements)]
    ...
    def partition_previous_reduced_grads(self):
        ...
        if self.grads_in_partition is None:
            ...
            for i, group in enumerate(self.fp16_groups):
                ...
                see_memory_usage(f"before copying {total_size} gradients into partition", force=False)
                self.grads_in_partition.append(torch.empty(int(total_size),
                                                  dtype=torch.half,
                                                  device=self.device))
                see_memory_usage(f"after copying {total_size} gradients into partition", force=False)
        ...
        for param in self.previous_reduced_grads:
            ...
            grad_tensor = self.grads_in_partition[i].narrow(
                        0,
                        dest_offset,
                        num_elements)
            ...
            if self.cpu_offload:
                ...
                if self.is_gradient_accumulation_boundary:
                    ...
                    fp32_grad_tensor = self.fp32_partitioned_groups_flat[i].grad.narrow(0,
                                                                    dest_offset,
                                                                    num_elements)

                    self.async_inplace_copy_grad_to_fp32_buffer_from_gpu(param, fp32_grad_tensor)
            else:
                param.partition_gradients(partition_buffers=grad_tensor,
        ...
        for i, group in enumerate(self.fp16_groups):
            ...
            if self.cpu_offload:
                ...
                single_grad_partition = _flatten_dense_tensors(
                    self.averaged_gradients[i]).to(self.fp32_partitioned_groups_flat[i].dtype)
        ...
        for i, group in enumerate(self.optimizer.param_groups):
            ...
            for key, value in self.optimizer.state[p].items():
                if torch.is_tensor(value):
                    padded_lens = [t.numel() for t in self.fp16_groups[i]]
                    lean_state[key] = self._get_lean_tensors(value,
                                                             self.fp16_groups[i],
    ...
    def state_dict(self):
        ...
        state_dict['base_optimizer_state'] = self.get_lean_optimizer_state()

        state_dict['partition_count'] = self.partition_count

        # Remove paddings for DP alignment to enable loading for other alignment values
        fp32_groups_without_padding = self.get_groups_without_padding(
            self.fp32_partitioned_groups_flat)
        state_dict['fp32_groups'] = fp32_groups_without_padding
        return state_dict
